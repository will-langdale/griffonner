...
"""
    ...
    @pytest.mark.parametrize(
        ["extension", "with_frontmatter"],
        [
            pytest.param("md", True, id="markdown_with_frontmatter"),
            pytest.param("json", True, id="json_with_frontmatter"),
            pytest.param("html", True, id="html_with_frontmatter"),
            pytest.param("txt", True, id="text_with_frontmatter"),
            pytest.param("md", False, id="markdown_without_frontmatter"),
            pytest.param("json", False, id="json_without_frontmatter"),
        ],
    )
    def test_finds_files_with_frontmatter_any_extension(
        self, extension: str, with_frontmatter: bool
    ):
        """Tests that files with frontmatter are found regardless of extension."""
        with TemporaryDirectory() as temp_dir:
            temp_path = Path(temp_dir)
            ...
            if with_frontmatter:
                ...
                assert len(found_files) == 0
                assert test_file not in found_files
